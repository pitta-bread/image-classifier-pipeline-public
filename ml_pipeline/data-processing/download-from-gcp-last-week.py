...
destination_folder = f"data/last-week-tapo-{datetime.date.today().strftime('%Y%m%d')}"
os.makedirs(destination_folder, exist_ok=True)

# find size of the last week's files in bucket
blobs = storage_client.list_blobs(bucket_name, start_offset=f"copper_cam_sd_stream_{time.time() - 7 * 24 * 60 * 60}")
total_size = 0
for blob in blobs:
    # print(blob.name)
    # print(blob.size)
...
# download the last week's files in bucket
blobs = storage_client.list_blobs(bucket_name, start_offset=f"copper_cam_sd_stream_{time.time() - 7 * 24 * 60 * 60}")
download_progress = 0
